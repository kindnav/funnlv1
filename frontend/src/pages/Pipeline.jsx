...
const STAGES = [
  { key: 'New',           label: 'Inbox',         color: '#7c6dfa', dot: 'bg-[#7c6dfa]' },
  { key: 'Reviewed',      label: 'In Review',      color: '#f5a623', dot: 'bg-[#f5a623]' },
  { key: 'In Diligence',  label: 'In Diligence',   color: '#4da6ff', dot: 'bg-[#4da6ff]' },
  { key: 'Passed',        label: 'Passed',          color: '#3dd68c', dot: 'bg-[#3dd68c]' },
];
...
export default function Pipeline({ user, onLogout }) {
  ...
  const grouped = STAGE_KEYS.reduce((acc, key) => {
    acc[key] = deals.filter((d) => d.status === key);
    return acc;
  }, {});
