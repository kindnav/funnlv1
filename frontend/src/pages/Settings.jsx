        ...
        <div className="max-w-2xl mx-auto space-y-5">
          ...
          <div className={cardCls} data-testid="gate-tests-section">
            <div className="flex items-center justify-between mb-1">
              ...
              >
                {gateTestRunning ? <RefreshCw size={11} className="animate-spin" /> : <ChevronRight size={11} />}
                  ...
                  {gateTestResults.results.map((r) => (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[rgba(255,255,255,0.25)] text-xs mt-0.5">
                          Expected: <span style={{ color: r.expected ? '#3dd68c' : '#f05252' }}>
                            {r.expected ? 'pass through' : 'filter out'}
                          </span>
                          {' · '}
                          Got: <span style={{ color: r.actual ? '#3dd68c' : '#f05252' }}>
          ...
          <div className={cardCls} data-testid="filtered-emails-section">
            ...
            <p className="text-[rgba(255,255,255,0.35)] text-xs mb-4 leading-relaxed">
              These emails were filtered by AI because they did not appear to be deal-relevant. The bar to filter is very high — only clear internal logistics emails are excluded. If a real deal was filtered by mistake, click Restore.
            </p>

            {gatedTableMissing && (
              <div className="rounded-lg px-4 py-3 mb-4" style={{ background: 'rgba(245,166,35,0.08)', border: '1px solid rgba(245,166,35,0.2)' }}>
  ...
  created_at timestamptz DEFAULT now()
            ...
            {gatedLoading ? (
              <div className="flex items-center gap-2 py-4">
                <div className="w-4 h-4 rounded-full border border-t-transparent animate-spin" style={{ borderColor: 'rgba(255,255,255,0.2)', borderTopColor: '#7c6dfa' }} />
                <span className="text-[rgba(255,255,255,0.3)] text-xs">Loading...</span>
              </div>
            ) : gatedEmails.length === 0 ? (
              <p className="text-[rgba(255,255,255,0.25)] text-xs py-2">No filtered emails. Everything is passing through to your dashboard.</p>
            ) : (
              <div className="space-y-2">
                {gatedEmails.map((e) => (
                  <div
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        <p className="text-white text-xs font-medium truncate">{e.sender_name || e.sender_email}</p>
                    ...
                    <button
                      ...
                      style={{ background: 'rgba(124,109,250,0.1)', color: '#7c6dfa', border: '1px solid rgba(124,109,250,0.2)' }}
                    >
                      {restoringId === e.id ? <RefreshCw size={10} className="animate-spin" /> : <RotateCcw size={10} />}
                      Restore
                    </button>
                  </div>
                ))}
