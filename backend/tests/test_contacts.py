...
class TestGetContacts:
    ...
    def test_get_contacts_returns_200(self) -> None:
    ...
    def test_get_contacts_unauthorized(self) -> None:
...
class TestUpsertContact:
    ...
    def test_deduplication_same_email_returns_updated(self) -> None:
        payload = {"deal": SAMPLE_DEAL, "contact_status": "In Pipeline"}
        r = requests.post(f"{BASE_URL}/api/contacts/upsert", json=payload, headers=HEADERS)
        assert r.status_code == 200, f"Expected 200, got {r.status_code}: {r.text}"
        data = r.json()
    ...
    def test_upsert_in_review_status(self) -> None:
...
class TestUpdateContact:
    ...
    def test_update_status(self) -> None:
        ...
        assert r.status_code == 200, f"Expected 200, got {r.status_code}: {r.text}"
        print("PATCH contact status: ✓")


class TestGetContactDeals:
    """GET /api/contacts/{id}/deals"""

    def test_get_contact_deals(self) -> None:
        global created_contact_id
        if not created_contact_id:
            pytest.skip("No contact id")
    ...
    def test_get_contact_deals_invalid_id(self) -> None:
